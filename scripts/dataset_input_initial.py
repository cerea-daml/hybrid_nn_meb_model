...
def main(args: argparse.Namespace, data_type: str = "train"):
    ...
    ds_forecast = ds_forecast.sel(lead_time=["0min 0s"])
    ...
    mean_path = os.path.join(
        args.data_path, "train", "climatology", "input_initial_mean.nc"
    )
    std_path = os.path.join(
        args.data_path, "train", "climatology", "input_initial_std.nc"
    )
    if data_type == "train":
    ...
    normalised_data.to_zarr(
        os.path.join(data_type_path, "dataset", "input_initial"),
        mode="w"
    )
