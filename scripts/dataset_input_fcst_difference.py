...
def main(args: argparse.Namespace, data_type: str = "train"):
    ...
    ds_input[{"lead_time": 0}] = ds_input.isel(lead_time=1)-ds_input.isel(lead_time=0)
    ...
    mean_path = os.path.join(
        args.data_path, "train", "climatology", "input_fcst_difference_mean.nc"
    )
    std_path = os.path.join(
        args.data_path, "train", "climatology", "input_fcst_difference_std.nc"
    )
    if data_type == "train":
    ...
    normalised_data.to_zarr(
        os.path.join(data_type_path, "dataset", "input_fcst_difference"),
        mode="w"
    )
