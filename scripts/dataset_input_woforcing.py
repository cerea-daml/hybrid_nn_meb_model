...
def main(args: argparse.Namespace, data_type: str = "train"):
    ...
    mean_path = os.path.join(
        args.data_path, "train", "climatology", "input_woforcing_mean.nc"
    )
    std_path = os.path.join(
        args.data_path, "train", "climatology", "input_woforcing_std.nc"
    )
    if data_type == "train":
    ...
    normalised_data.to_zarr(
        os.path.join(data_type_path, "dataset", "input_woforcing"),
        mode="w"
    )
